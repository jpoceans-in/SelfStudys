...
# describe and delete PrivateHostedZone
aws cloudformation describe-stacks --region ${pRegion} --stack-name "$pProject-$pEnvironment-PrivateHostedZone" --query "Stacks[0].{Name:StackName,Status:StackStatus,CreationTime:CreationTime}"
aws cloudformation delete-stack --region ${pRegion} --stack-name "$pProject-$pEnvironment-PrivateHostedZone"


# wait for NaclEntryStackPrivate, NaclEntryStackPublic, SecurityGroupStack, IAMRolesStack, PrivateHostedZone
...
aws cloudformation wait stack-delete-complete --region ${pRegion} --stack-name "$pProject-$pEnvironment-PrivateHostedZone"
